...
\uncover<13->{\mathbb{Q}} &\uncover<14->{= \left\{\frac{p}{q} : p, q \in \mathbb{Z} \text{ and } q \neq 0\right\}}
  \uncover<15->{\qquad (\text{``rational numbers''})} \\
\uncover<16->{\mathbb{R}} &\uncover<17->{= \{\text{decimal numbers}\}}
  \uncover<18->{\qquad\quad (\text{``real numbers''})}
\end{split}
\]
